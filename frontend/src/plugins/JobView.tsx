...
export default function JobView() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0b0e14] overflow-hidden">{renderContent()}</div>
  );
}
...
function Dashboard({ onSelect }: { onSelect: (m: JobMode) => void }) {
  ...
  return (
    <div className="flex-1 p-8 overflow-y-auto custom-scrollbar">
      <div className="mb-12">
        <h1 className="text-3xl font-light text-white mb-2">{t.job_title}</h1>
        <p className="text-gray-500 text-sm">{t.job_subtitle}</p>
      </div>
      ...
      <section>
        <h3 className="text-sm font-bold text-gray-600 uppercase tracking-[0.2em] mb-6">
        ...
        <div className="glass-card rounded-[12px] p-8 border border-white/5 bg-gradient-to-br from-[#2979ff]/10 to-transparent relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <h2 className="text-2xl text-white mb-3">{t.job_aiInterview}</h2>
            <p className="text-gray-400 max-w-xl mb-8">{t.job_aiInterviewDesc}</p>

            <div className="flex items-center space-x-4">
              <button
                type="button"
                onClick={() => onSelect('interview')}
                className="flex items-center px-8 py-3 bg-[#2979ff] text-white rounded-[4px] text-xs font-bold hover:bg-blue-600 transition-all shadow-lg shadow-blue-500/20 uppercase tracking-widest"
              >
    ...
    <div className="flex-1 flex flex-col p-8">
      <div className="flex items-center justify-between mb-12">
        <button
          type="button"
          onClick={onBack}
          className="text-[10px] font-bold text-gray-500 hover:text-white uppercase tracking-widest flex items-center"
        >
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full text-center">
        {step === 'intro' && (
          <div>
            ...
            <h2 className="text-3xl font-light text-white mb-4">{t.job_recruiterSim}</h2>
            <p className="text-gray-500 mb-12">{t.job_recruiterIntro}</p>

            <div className="grid grid-cols-2 gap-4 mb-12">
              <div className="p-4 bg-white/2 border border-white/5 rounded-[4px] text-left">
                <p className="text-[10px] font-bold text-gray-600 uppercase mb-1">{t.job_metrics}</p>
                <p className="text-xs text-gray-300">{t.job_metricsDesc}</p>
              </div>
              <div className="p-4 bg-white/2 border border-white/5 rounded-[4px] text-left">
                <p className="text-[10px] font-bold text-gray-600 uppercase mb-1">{t.job_duration}</p>
            ...
            <button
              ...
              className="px-12 py-4 bg-[#2979ff] text-white rounded-[4px] text-xs font-bold hover:bg-blue-600 transition-all shadow-xl shadow-blue-500/20 uppercase tracking-widest"
            >
        ...
        {step === 'session' && (
          <div className="w-full">
            ...
            <p className="text-xl text-white mb-4">{t.job_sampleQuestion}</p>
        ...
        {step === 'feedback' && (
          <div className="w-full text-left">
            <div className="flex items-center space-x-3 mb-8">
              ...
              <h2 className="text-2xl text-white font-light">{t.job_performanceReport}</h2>
            </div>
